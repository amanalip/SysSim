...
export function parseSnapshotSlots(serialized: string | null): SnapshotSlot[] {
  if (!serialized)
    return Array.from({ length: SNAPSHOT_SLOT_COUNT }, (_, index) => emptySnapshotSlot(index + 1));
  let parsed: unknown;
  try {
    ...
    return [
      {
        ...emptySnapshotSlot(1),
        corrupted: true,
        corruptionReason: 'Stored snapshot JSON is malformed',
      },
      ...Array.from({ length: 4 }, (_, index) => emptySnapshotSlot(index + 2)),
    ];
  }
    ...
    return [
      {
        ...emptySnapshotSlot(1),
        corrupted: true,
        corruptionReason: 'Stored snapshot collection is not an array',
      },
      ...Array.from({ length: 4 }, (_, index) => emptySnapshotSlot(index + 2)),
    ];
  ...
  parsed.slice(0, SNAPSHOT_SLOT_COUNT).forEach((candidate, index) => {
    ...
    try {
      ...
      const id =
        Number.isInteger(raw.id) && Number(raw.id) >= 1 && Number(raw.id) <= SNAPSHOT_SLOT_COUNT
          ? Number(raw.id)
          : index + 1;
      ...
      byId.set(index + 1, {
        ...fallback,
        corrupted: true,
        corruptionReason: formatArchitectureError(error),
      });
    }
  });
  return Array.from(
    { length: SNAPSHOT_SLOT_COUNT },
    (_, index) => byId.get(index + 1) || emptySnapshotSlot(index + 1),
  );
}

export function persistSnapshotSlots(
...
export function importSnapshotSlots(serialized: string): SnapshotSlot[] {
  ...
  if (corrupt)
    throw new Error(`Snapshot slot ${corrupt.id} is invalid: ${corrupt.corruptionReason}`);
  return slots;
}
