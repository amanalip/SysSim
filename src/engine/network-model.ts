...
export const PROTOCOL_ASSUMPTIONS: Record<
  EdgeProtocol,
  { overheadMs: number; modeledGuarantees: string }
> = {
  HTTP: { overheadMs: 1.5, modeledGuarantees: 'request/response timing only' },
  gRPC: { overheadMs: 0.7, modeledGuarantees: 'framing overhead only' },
  WebSocket: { overheadMs: 0.4, modeledGuarantees: 'established-channel timing only' },
  TCP: { overheadMs: 0.3, modeledGuarantees: 'stream transfer timing only' },
  UDP: { overheadMs: 0.1, modeledGuarantees: 'no delivery or ordering guarantee' },
  MQTT: { overheadMs: 0.5, modeledGuarantees: 'publish timing; broker semantics are separate' },
  'pub/sub': {
    overheadMs: 0.8,
...
export function calculateNetworkTransfer(input: NetworkTransferInput): NetworkTransferResult {
  ...
  const crossZoneCost = crossZone
    ? (payloadKb / (1024 * 1024)) * Math.max(0, input.crossZoneCostPerGb || 0)
    : 0;
