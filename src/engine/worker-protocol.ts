...
export function isWorkerCommand(value: unknown): value is WorkerCommand {
  ...
  if (['START', 'PAUSE', 'RESUME', 'STEP', 'STOP', 'RESET', 'DISPOSE'].includes(message.type)) return true;
  if (message.type === 'SET_SPEED') return typeof message.payload === 'number' && Number.isFinite(message.payload);
  if (message.type === 'UPDATE_CONFIG') return Boolean(message.payload && typeof message.payload === 'object');
  if (message.type === 'INIT_OR_UPDATE_GRAPH') {
    const payload = message.payload as { graph?: SimGraph; graphRevision?: number } | undefined;
    return Boolean(payload && Number.isInteger(payload.graphRevision) && payload.graph &&
...
export function isWorkerResponse(value: unknown): value is WorkerResponse {
  ...
  if (message.type === 'GRAPH_ACK') {
    return Number.isInteger((message.payload as { graphRevision?: unknown } | undefined)?.graphRevision);
  }
  if (message.type !== 'TICK_UPDATE' || !message.payload || typeof message.payload !== 'object') return false;
  const payload = message.payload as Partial<TickPayload>;
  return Number.isInteger(payload.graphRevision) && Boolean(payload.metrics) &&
    Array.isArray(payload.activeRequests) && Array.isArray(payload.recentRequests);
}
