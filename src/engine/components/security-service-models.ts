...
export class AuthServiceModel {
  ...
  public validate() {
    ...
    if (this.config.tokenType === 'Session' && this.config.sessionCacheEnabled) {
      cached = this.random() * 100 < Math.min(100, Math.max(0, this.config.sessionCacheHitRatePercent));
      if (cached) {
        this.cacheHits++;
      ...
      const tokenFactor = this.config.tokenType === 'Paseto' ? 1.15 : this.config.tokenType === 'Session' ? 1.5 : 1;
  ...
  public getMetrics() {
    return {
      ...
      averageValidationLatencyMs: this.validations ? this.validationLatencyTotalMs / this.validations : 0,
    };
  }

  public reset() { this.cacheHits = 0; this.cacheMisses = 0; this.validationLatencyTotalMs = 0; this.validations = 0; }
}
...
export class EncryptionServiceModel {
  ...
  public process(payloadSizeKb: number) {
    ...
    const algorithmFactor = this.config.algorithm === 'RSA-4096' ? 6 : this.config.algorithm === 'ChaCha20-Poly1305' ? 0.8 : 1;
    const perKbMs = this.config.algorithm === 'RSA-4096' ? 0.02 : this.config.algorithm === 'ChaCha20-Poly1305' ? 0.0015 : 0.002;
    const latencyMs = Math.max(0, this.config.overheadLatencyMs) * algorithmFactor + payloadKb * perKbMs;
    this.operations++;
  ...
  public reset() { this.operations = 0; this.latencyTotalMs = 0; this.payloadKb = 0; }
}
