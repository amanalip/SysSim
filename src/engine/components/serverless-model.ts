...
export class ServerlessModel {
  ...
  private warmStarts = 0;
  private timeouts = 0;
  private throttles = 0;
  private invocationFailures = 0;
  private downstreamFailures = 0;
  private totalExecutionLatencyMs = 0;
  private lastColdProbabilityPercent = 100;
  ...
  constructor(
    ...
    private baseExecutionLatencyMs: number,
    private warmInstances: number,
  ...
  public invoke(arrivalTimeMs: number): ServerlessInvocationResult {
    ...
    const coldProbability =
      provisionedWarm || stillWarm
        ? 0
        : !slot.initialized
          ? 1
          : Math.min(1, idleMs / idleTimeoutMs);
    ...
    const executionLatencyMs =
      Math.max(0, this.baseExecutionLatencyMs) * Math.sqrt(512 / Math.max(128, this.memoryMb));
    ...
    if (coldStart) this.coldStarts++;
    else this.warmStarts++;
    if (timedOut) {
      this.timeouts++;
    ...
    return {
      ...
      coldStartLatencyMs,
      queueLatencyMs,
      totalLatencyMs: queueLatencyMs + billedLatencyMs,
      coldStart,
      coldStartProbabilityPercent: this.lastColdProbabilityPercent,
      timedOut,
      throttled: false,
      activeInvocations: metrics.activeInvocations,
      queuedInvocations: metrics.queuedInvocations,
    };
  }

  public recordInvocationFailure(): void {
    this.invocationFailures++;
  }
  public recordDownstreamFailure(): void {
    this.downstreamFailures++;
  }
  ...
  public getMetrics(nowMs: number): ServerlessMetrics {
    ...
    return {
      ...
      utilizationPercent: Math.min(
        100,
        Math.round((activeInvocations / this.capacity) * 1000) / 10,
      ),
      throttles: this.throttles,
      invocationFailures: this.invocationFailures,
      downstreamFailures: this.downstreamFailures,
    };
  }

  public reset(): void {
    this.slots = Array.from({ length: this.provisionedWarmCount }, () => ({
      availableAtMs: 0,
      warmUntilMs: Number.POSITIVE_INFINITY,
      initialized: true,
    }));
    this.coldStarts = 0;
    this.warmStarts = 0;
    this.timeouts = 0;
    this.throttles = 0;
    this.invocationFailures = 0;
    this.downstreamFailures = 0;
    this.totalExecutionLatencyMs = 0;
    this.lastColdProbabilityPercent = 100;
  }

  private findAvailableSlot(nowMs: number): number {
    return this.slots.findIndex((slot) => slot.availableAtMs <= nowMs);
  }
  private get capacity(): number {
    return Math.max(1, Math.floor(this.concurrencyLimit || 1));
  }
  private get provisionedWarmCount(): number {
    return Math.min(this.capacity, Math.max(0, Math.floor(this.warmInstances || 0)));
  }
}
