...
interface Reservation { id: number; endAtMs: number }
...
export class ReverseProxyModel {
  ...
  public updateConfig(config: ReverseProxyConfig): void { this.config = config; }

  public begin(nowMs: number, payloadKb: number): {
    accepted: boolean; reservationId?: number; latencyMs: number; outputPayloadKb: number; backpressureMs: number;
  } {
    ...
    const overflowKb = this.config.bufferingEnabled ? Math.max(0, outputPayloadKb - this.config.bufferSizeKb) : outputPayloadKb;
  ...
  public finish(reservationId: number | undefined, nowMs: number, downstreamLatencyMs: number): void {
  ...
  public getMetrics(nowMs: number) {
    this.reservations = this.reservations.filter((item) => item.endAtMs > nowMs);
    return { activeConnections: this.reservations.length, rejected: this.rejected, compressedKbSaved: this.compressedKbSaved, backpressureMs: this.backpressureMs };
  }

  public reset(): void { this.reservations = []; this.sequence = 0; this.rejected = 0; this.compressedKbSaved = 0; this.backpressureMs = 0; }
}
