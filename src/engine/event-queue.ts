...
export class EventPriorityQueue {
  ...
  public kinds(): SimulationEventKind[] {
    return this.heap.map((event) => event.kind);
  }
  public clear(): void {
    this.heap = [];
    this.nextId = 0;
  ...
  private before(a: SimulationEvent, b: SimulationEvent): boolean {
    return a.timeMs < b.timeMs || (a.timeMs === b.timeMs && a.id < b.id);
  }
