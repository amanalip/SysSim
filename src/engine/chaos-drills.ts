...
export type ChaosDrillId =
  'db_outage' | 'cache_stampede' | 'flash_crowd' | 'ingress_partition' | 'network_latency';
...
export class ChaosDrillManager {
  ...
  public getActiveRecords(): ChaosDrillRecord[] {
    return [...this.active.values()].map((item) => item.record);
  }
  public getHistory(): ChaosDrillRecord[] {
    return [...this.history];
  }

  public launch(
    id: ChaosDrillId,
    options: { stampedeProtection?: boolean } = {},
  ): ChaosDrillRecord {
    if (this.active.size > 0)
      return this.failure(
        id,
        'Restore the active drill before starting another; overlapping restoration is intentionally blocked.',
      );
    ...
    if (id === 'db_outage') {
      const databases = state.nodes.filter(
        (node) => node.data.config.type === 'sql_db' || node.data.config.type === 'nosql_db',
      );
      const primary = databases[0];
      if (!primary)
        return this.failure(id, 'No database exists, so no failover target can be exercised.');
      const internalFailover =
        primary.data.config.type === 'sql_db' &&
        primary.data.config.automaticFailover &&
        (primary.data.config.readReplicasCount > 0 || primary.data.config.replicas > 1);
      const alternate = databases
        .slice(1)
        .find((candidate) =>
          state.edges.some(
            (edge) =>
              edge.target === candidate.id &&
              state.edges.some((peer) => peer.source === edge.source && peer.target === primary.id),
          ),
        );
      if (!internalFailover && !alternate)
        return this.failure(
          id,
          'Primary database has no configured replica or separately connected failover target.',
        );
      ...
      injectedParameters = {
        failureMode: internalFailover ? 'internal_replica_failover' : 'topology_failover',
      };
      observedResult = internalFailover
        ? 'Primary degraded and SQL automatic failover path activated.'
        : `Primary down; traffic can continue through ${alternate!.data.config.name}.`;
      ...
      for (const cache of caches) {
        ...
        if ('requestCoalescingEnabled' in cache.data.config)
          partial.requestCoalescingEnabled = Boolean(options.stampedeProtection);
      ...
      injectedParameters = {
        ...
        stampedeProtection: Boolean(options.stampedeProtection),
      };
      observedResult = options.stampedeProtection
        ? 'Caches bypassed; concurrent misses are coalesced before origin fill.'
        : 'Caches bypassed; every miss is forwarded to origin traffic.';
      ...
      injectedParameters = {
        ...
        originalQps: baseQps,
        injectedQps,
        preservedPattern: state.trafficConfig.pattern,
      };
      ...
      state.setEdges((edges) =>
        edges.map((candidate) =>
          candidate.id === edge.id
            ? { ...candidate, data: { ...candidate.data, isCut: true } }
            : candidate,
        ),
      );
      ...
      const ingress = this.findIngressEdges(state.edges);
      const targets = ingress.length
        ? ingress
        : state.edges.filter(
            (edge) => getEdgePurpose(edge.data) === 'request' && !edge.data?.isCut,
          );
      if (!targets.length)
        return this.failure(id, 'No active request edge exists for latency injection.');
      ...
      state.setEdges((edges) =>
        edges.map((edge) =>
          ids.has(edge.id)
            ? {
                ...edge,
                data: { ...edge.data, latencyMs: Math.max(0, edge.data?.latencyMs || 0) + 400 },
              }
            : edge,
        ),
      );
  ...
  public restore(id: ChaosDrillId): boolean {
    ...
    const state = useStore.getState();
    for (const [nodeId, health] of Object.entries(point.health))
      state.setNodeHealthOverride(nodeId, health, 'manual');
    state.updateNodeConfigs(point.configs);
    if (Object.keys(point.edges).length)
      state.setEdges((edges) =>
        edges.map((edge) =>
          point.edges[edge.id] ? { ...edge, data: structuredClone(point.edges[edge.id]) } : edge,
        ),
      );
    if (point.traffic) {
  ...
  public restoreAll(): void {
    for (const id of [...this.active.keys()]) this.restore(id);
  }
