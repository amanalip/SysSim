...
export function compareArchitectures(user: SerializedCanvasState, reference: SerializedCanvasState): ArchitectureComparison {
  ...
  return {
    ...
    userNodeCount: user.nodes.length, referenceNodeCount: reference.nodes.length,
    userEdgeCount: user.edges.length, referenceEdgeCount: reference.edges.length,
    guidance: 'Differences are discussion prompts, not correctness failures. Compare responsibilities, reachable paths, bottlenecks, and failure behavior rather than visual layout.',
  };
}
