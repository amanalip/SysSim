...
function inferSourceType(
  title: string,
  organization: string,
): NonNullable<Scenario['sources'][number]['sourceType']> {
  ...
  if (
    /engineering|documentation|docs|developer|architecture|aws|google|microsoft|cloudflare|netflix|uber|meta|discord|slack/.test(
      value,
    )
  )
    return 'official';
...
export function normalizeScenario(scenario: Scenario): Scenario {
  return {
    ...
    trafficPreset: {
      ...scenario.trafficPreset,
      baseQps: Math.min(50_000, scenario.trafficPreset.baseQps),
    },
    sources: scenario.sources.map((source) => ({
      ...
      supports:
        source.supports ||
        `Background evidence for the terminology, trade-offs, or architecture discussion in “${scenario.title}”.`,
