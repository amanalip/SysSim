...
const golden: CalculatorInputs = {
  qps: 1_000, payloadSizeKb: 2, retentionDays: 1, readWriteRatio: 1, replicationFactor: 1,
  slaAvailabilityPercent: 99.9, serverCapacityQps: 500,
  readRequestPayloadKb: 0.5, readResponsePayloadKb: 2, writeResponsePayloadKb: 0.2,
  dbAverageServiceTimeMs: 20, dbTargetUtilizationPercent: 100,
  cacheWorkingSetDays: 1, cacheHotSetPercent: 20, cacheCompressionRatio: 1,
  serverTargetUtilizationPercent: 100, serverHeadroomPercent: 0, failoverCapacityPercent: 0,
  indexingOverheadPercent: 0, metadataOverheadPercent: 0, storageCompressionRatio: 1, annualGrowthPercent: 0,
};
...
describe('production capacity calculator tasks 285-297', () => {
  it('matches a hand-calculated decimal-SI golden reference', () => {
    ...
    expect(output).toMatchObject({ readQps: 500, writeQps: 500, dailyNewDataGb: 86.4, totalStorageNeededTb: 0.09,
      totalReplicatedStorageTb: 0.09, inboundBandwidthMbps: 10, outboundBandwidthMbps: 8.8,
      estimatedServersNeeded: 2, recommendedCacheMemoryGb: 17.3, estimatedDbConnections: 20 });
  });

  it('uses total QPS and independent request/response payload assumptions', () => {
    const output = calculateCapacity({ ...golden, readWriteRatio: 3, readRequestPayloadKb: 1, readResponsePayloadKb: 4 });
  ...
  it('models headroom, failover reserve, and utilization in server count', () => {
    ...
    const protectedCount = calculateCapacity({ ...golden, serverTargetUtilizationPercent: 50, serverHeadroomPercent: 25, failoverCapacityPercent: 25 }).estimatedServersNeeded;
  ...
  it('models database concurrency with service time and target utilization', () => {
    ...
    expect(calculateCapacity({ ...golden, dbAverageServiceTimeMs: 40, dbTargetUtilizationPercent: 50 }).estimatedDbConnections).toBe(80);
  });

  it('models cache working set and storage overhead independently', () => {
    const modeled = calculateCapacity({ ...golden, cacheWorkingSetDays: 2, cacheHotSetPercent: 50, cacheCompressionRatio: 0.5,
      indexingOverheadPercent: 20, metadataOverheadPercent: 10, storageCompressionRatio: 0.5, annualGrowthPercent: 20, replicationFactor: 3 });
  ...
  it('handles zero, negative, non-finite, and extreme boundaries without NaN or Infinity', () => {
    const output = calculateCapacity({ ...golden, qps: Number.NaN, payloadSizeKb: -1, serverCapacityQps: 0, storageCompressionRatio: Infinity });
