...
const traffic = (baseQps: number, seed = 19): TrafficConfig => ({
  pattern: 'steady',
  baseQps,
  burstMultiplier: 1,
  rampDurationSec: 1,
...
describe('discrete-event tasks 179-185', () => {
  it('orders every event category by timestamp and stable insertion order', () => {
    ...
    const kinds: SimulationEventKind[] = [
      'arrival',
  ...
  it('keeps a request genuinely in flight until its scheduled completion', () => {
    const graph: SimGraph = {
      nodes: [
        ...
        { id: 'app', config: createDefaultConfig('app_server', 'app') },
      ],
      edges: [
        {
          id: 'slow-link',
          source: 'client',
          target: 'app',
          data: { protocol: 'HTTP', purpose: 'request', latencyMs: 1000 },
        },
      ],
    };
    ...
    expect(engine.getPendingEventKinds()).toEqual(
      expect.arrayContaining(['edge_transfer', 'node_service_completion', 'request_completion']),
    );
    const completed = engine.step(1000);
    expect(completed.activeRequests.some((request) => request.id === firstRequestId)).toBe(false);
    expect(completed.recentRequests.find((request) => request.id === firstRequestId)?.status).toBe(
      'success',
    );
  });

  it('joins concurrent fanout branches at the slowest branch rather than summing them', () => {
    const app = (id: string, processingLatencyMs: number) => ({
      ...createDefaultConfig('app_server', id),
      processingLatencyMs,
    });
    const engine = new SysSimEngine(
      {
        nodes: [
          { id: 'root', config: app('root', 20) },
          { id: 'fast', config: app('fast', 10) },
          { id: 'slow', config: app('slow', 30) },
        ],
        edges: [
          {
            id: 'fast-edge',
            source: 'root',
            target: 'fast',
            data: { protocol: 'HTTP', purpose: 'fanout', latencyMs: 0 },
          },
          {
            id: 'slow-edge',
            source: 'root',
