...
const edge = (
  source: string,
  target: string,
  purpose: ProtocolEdgeData['purpose'] = 'request',
) => ({
  id: `${source}-${target}-${purpose}`,
  source,
  target,
  data: { protocol: 'HTTP' as const, purpose },
});
...
describe('graph-aware bottleneck detection tasks 263-275', () => {
  it('does not flag isolated or unused components', () => {
    const nodes = [
      node('client', 'client'),
      node('used', 'app_server', { replicas: 2 }),
      node('isolated', 'sql_db'),
    ];
  ...
  it('finds a reachable dominating SPOF and ignores a redundant route', () => {
    const nodes = [
      node('client', 'client'),
      node('gateway', 'api_gateway'),
      node('app', 'app_server', { replicas: 2 }),
      node('db', 'sql_db', { replicas: 2 }),
    ];
    const linear = detectBottlenecks(nodes, [
      edge('client', 'gateway'),
      edge('gateway', 'app'),
      edge('app', 'db'),
    ]);
    expect(
      linear.find((issue) => issue.type === 'spof' && issue.nodeId === 'gateway')?.triggerPath,
    ).toEqual(['client', 'gateway']);
    const redundant = detectBottlenecks(nodes, [
      edge('client', 'gateway'),
      edge('client', 'app'),
      edge('gateway', 'db'),
      edge('app', 'db'),
    ]);
    expect(redundant.some((issue) => issue.type === 'spof' && issue.nodeId === 'gateway')).toBe(
      false,
    );
  });
  ...
  it('requires a reachable read-heavy database path without a cache', () => {
    const readNodes = [
      node('client', 'client', { readPercentage: 80 }),
      node('app', 'app_server'),
      node('db', 'sql_db'),
    ];
    expect(
      detectBottlenecks(readNodes, [edge('client', 'app'), edge('app', 'db')]).some(
        (issue) => issue.type === 'missing_cache',
      ),
    ).toBe(true);
    const writeNodes = [
      node('client', 'client', { readPercentage: 10 }),
      node('app', 'app_server'),
      node('db', 'sql_db'),
    ];
    expect(
      detectBottlenecks(writeNodes, [edge('client', 'app'), edge('app', 'db')]).some(
        (issue) => issue.type === 'missing_cache',
      ),
    ).toBe(false);
    expect(
      detectBottlenecks(readNodes.concat(node('cache', 'redis_cache')), [
        edge('client', 'app'),
        edge('app', 'cache'),
        edge('cache', 'db'),
      ]).some((issue) => issue.type === 'missing_cache'),
    ).toBe(false);
  });

  it('traverses actual synchronous paths and excludes replication and observability links', () => {
    const nodes = ['client', 'a', 'b', 'c', 'd'].map((id, index) =>
      node(id, index ? 'app_server' : 'client', { replicas: 2 }),
    );
    const requestEdges = [edge('client', 'a'), edge('a', 'b'), edge('b', 'c'), edge('c', 'd')];
    expect(
      detectBottlenecks(nodes, requestEdges).some((issue) => issue.type === 'synchronous_chain'),
    ).toBe(true);
    const semanticEdges = [
      edge('client', 'a'),
      edge('a', 'b', 'replication'),
      edge('b', 'c', 'observability'),
      edge('c', 'd'),
    ];
    expect(
      detectBottlenecks(nodes, semanticEdges).some((issue) => issue.type === 'synchronous_chain'),
    ).toBe(false);
  });
  ...
  it('detects, deduplicates, explains, and ranks telemetry findings', () => {
    const nodes = [
      ...
      node('db', 'sql_db', { connectionQueueLimit: 100 }),
    ];
    ...
    expect(new Set(issues.map((issue) => `${issue.type}:${issue.nodeId}`)).size).toBe(
      issues.length,
    );
  ...
  it('is cycle-safe', () => {
    ...
    expect(() =>
      detectBottlenecks(nodes, [edge('client', 'a'), edge('a', 'b'), edge('b', 'a')]),
    ).not.toThrow();
  });
});
