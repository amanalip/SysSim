...
describe('transparent cost estimation tasks 298-306', () => {
  ...
  it('maps provider profiles deterministically outside React', () => {
    const nodes = [
      node('app', 'app_server'),
      node('db', 'sql_db', { replicas: 1, readReplicasCount: 2 }),
    ];
    const aws = estimateArchitectureCost(nodes, 1_000, 'aws', false);
    const gcp = estimateArchitectureCost(nodes, 1_000, 'gcp', false);
    expect(aws.lineItems.find((item) => item.id === 'app')?.instanceType).toBe(
      ILLUSTRATIVE_PRICE_TABLE.app_server?.profiles.aws,
    );
    expect(gcp.lineItems.find((item) => item.id === 'app')?.instanceType).toBe(
      ILLUSTRATIVE_PRICE_TABLE.app_server?.profiles.gcp,
    );
    expect(aws.lineItems.find((item) => item.id === 'db')?.replicas).toBe(3);
  });

  it.each(COMPONENT_METADATA_LIST.map(({ type }) => [type]))(
    'maps %s for every provider with a rationale',
    (type) => {
  ...
  it('applies spot discounts only to eligible compute profiles', () => {
    ...
    expect(spot.lineItems.find((item) => item.id === 'app')?.unitMonthlyCost).toBeLessThan(
      regular.lineItems.find((item) => item.id === 'app')?.unitMonthlyCost || 0,
    );
    expect(spot.lineItems.find((item) => item.id === 'db')?.unitMonthlyCost).toBe(
      regular.lineItems.find((item) => item.id === 'db')?.unitMonthlyCost,
    );
  });
  ...
  it('reports managed service, redundancy, storage, request, and bandwidth costs separately', () => {
    ...
    expect(estimate.totalMonthly).toBe(
      Math.round(
        estimate.managedServiceCost +
          estimate.redundancyCost +
          estimate.capacityStorageCost +
          estimate.requestCost +
          estimate.bandwidthCost,
      ),
    );
  });
