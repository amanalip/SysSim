...
import { deriveHealthFromCapacity, getHealthBehavior, HEALTH_RECOVERY_CONTRACT } from '../engine/health-state';
import { SysSimEngine } from '../engine/simulator';
import { createSimRequest } from '../engine/request';

const traffic: TrafficConfig = { pattern: 'steady', baseQps: 0, burstMultiplier: 1, rampDurationSec: 1, spikeFrequencySec: 1, seed: 71 };
const representative: Record<ComponentCategory, ComponentType> = {
  compute: 'app_server', networking: 'load_balancer', storage: 'sql_db',
  caching: 'redis_cache', messaging: 'message_queue', security: 'auth_service',
};

describe('health-state tasks 152-159', () => {
  it('defines one behavior contract for every health state', () => {
    expect(getHealthBehavior('healthy')).toMatchObject({ capacityMultiplier: 1, latencyMultiplier: 1, acceptsNewWork: true });
    expect(getHealthBehavior('degraded')).toMatchObject({ capacityMultiplier: 0.7, latencyMultiplier: 1.5, addedFailureRatePercent: 5 });
    expect(getHealthBehavior('overloaded')).toMatchObject({ capacityMultiplier: 0.5, latencyMultiplier: 2, addedFailureRatePercent: 10 });
  ...
  it.each(Object.entries(representative))('%s components reject work while down', (_category, type) => {
    ...
    (engine as unknown as { processRequest: (request: SimRequest) => void }).processRequest(request);
