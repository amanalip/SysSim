...
const traffic = {
  ...
  rampDurationSec: 1, spikeFrequencySec: 1, seed: 17,
};
...
const serverless = (overrides: Partial<ServerlessConfig> = {}): ServerlessConfig => ({
  ...
  concurrencyLimit: 1, warmInstances: 1, baseExecutionLatencyMs: 100,
  ...overrides,
});
...
const app = (id: string, overrides: Partial<AppServerConfig> = {}): AppServerConfig => ({
  ...
  replicas: 1, maxConnections: 1, processingLatencyMs: 100,
  ...overrides,
});

const loadBalancer = (algorithm: LoadBalancerConfig['algorithm']): LoadBalancerConfig => ({
  ...(createDefaultConfig('load_balancer', 'lb', 'LB') as LoadBalancerConfig), algorithm,
});

describe('serverless completion', () => {
  it('throttles excess invocations and reports them separately', () => {
    const engine = new SysSimEngine({ nodes: [{ id: 'function', config: serverless() }], edges: [] }, traffic);
  ...
  it('distinguishes platform invocation failure from downstream failure', () => {
    ...
    const downstreamFailure = new SysSimEngine({
      ...
      edges: [{ id: 'call', source: 'function', target: 'downstream', data: { protocol: 'HTTP', purpose: 'request' } }],
    }, traffic);
    execute(downstreamFailure, 'function', 1);
    expect(downstreamFailure.getMetricsSnapshot().componentMetrics.function).toMatchObject({
      serverlessInvocationFailures: 0, serverlessDownstreamFailures: 1,
    });
  });
});
...
describe('load-balancer completion', () => {
  it('tracks connection lifetimes so least-connections balances overlapping work', () => {
    ...
    const routes = [execute(engine, 'lb', 1), execute(engine, 'lb', 2)].map((request) => request.path[1].nodeId);
  ...
  it('excludes down targets and keys hashing by request or originating client as appropriate', () => {
    const makeEngine = (algorithm: LoadBalancerConfig['algorithm'], downA = false) => new SysSimEngine({
      nodes: [
    ...
    expect(execute(consistent, 'lb', 1, 'account:9').path[1].nodeId)
      .toBe(execute(consistent, 'lb', 2, 'account:9').path[1].nodeId);

    const ipHash = makeEngine('ip_hash');
    expect(execute(ipHash, 'lb', 1, 'first').path[1].nodeId)
      .toBe(execute(ipHash, 'lb', 2, 'second').path[1].nodeId);
  });
});
