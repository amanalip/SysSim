...
import { MessageQueueConfig, PubSubConfig, SimRequest, WorkerConfig } from '../model/types';
...
describe('messaging engine integration', () => {
  ...
  it('scales background drain with worker replicas, concurrency, and rate', () => {
    ...
    const small = run(
      workerConfig('worker', { replicas: 1, concurrencyLimit: 1, jobProcessingRatePerSec: 2 }),
    );
    const large = run(
      workerConfig('worker', { replicas: 3, concurrencyLimit: 2, jobProcessingRatePerSec: 2 }),
    );
    expect(small.componentMetrics.worker.totalRequests).toBe(2);
    expect(large.componentMetrics.worker.totalRequests).toBe(6);
    expect(large.componentMetrics.queue.queueDepth).toBeLessThan(
      small.componentMetrics.queue.queueDepth,
    );
  });
  ...
  it('fans one pub/sub publication out to the configured subscriber count', () => {
        ...
        edges: [
          {
            ...
            source: 'pubsub',
            target: 'worker-a',
            data: { protocol: 'pub/sub', purpose: 'fanout' },
          },
          {
            id: 'b',
            source: 'pubsub',
    ...
    expect(
      metrics.componentMetrics['worker-a'].totalRequests +
        metrics.componentMetrics['worker-b'].totalRequests,
    ).toBe(3);
