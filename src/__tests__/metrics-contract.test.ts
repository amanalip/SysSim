...
describe('metrics contract tasks 189-196', () => {
  ...
  it('matches a hand-calculated latency path and exposes its breakdown', () => {
    ...
    const engine = new SysSimEngine({
      nodes: [{ id: 'client', config: client }, { id: 'app', config: app }],
      edges: [{ id: 'edge', source: 'client', target: 'app', data: { protocol: 'HTTP', latencyMs: 10 } }],
    });
    ...
    expect((request.queueWaitMs || 0) + (request.serviceTimeMs || 0) + (request.networkTimeMs || 0))
  ...
  it('reports failed latency separately from successful percentiles', () => {
    ...
    engine.setGraph({ nodes: [{ id: 'client', config: { ...client, health: 'down' } }], edges: [] });
