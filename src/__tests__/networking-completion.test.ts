...
const traffic = {
  pattern: 'steady' as const, baseQps: 0, burstMultiplier: 1,
  rampDurationSec: 1, spikeFrequencySec: 1, seed: 23,
};

const execute = (engine: SysSimEngine, source: string, id: number, timestamp = 0, key = `key-${id}`) => {
...
describe('load balancer tasks 96-99', () => {
  it('detects failures on health-check intervals and delays recovery', () => {
    ...
    const graph: SimGraph = {
      nodes: [{ id: 'lb', config: loadBalancer({ healthCheckIntervalSec: 5, healthRecoveryDelaySec: 10 }) }, { id: 'a', config: target }],
      edges: [{ id: 'route', source: 'lb', target: 'a', data: { protocol: 'HTTP', purpose: 'request' } }],
    };
  ...
  it('keeps sticky clients together and applies editable target weights', () => {
    const graph: SimGraph = {
      nodes: [
        { id: 'c1', config: client('c1') }, { id: 'c2', config: client('c2') },
        { id: 'lb', config: loadBalancer({ stickySession: true, algorithm: 'weighted', targetWeights: { a: 3, b: 1 } }) },
        { id: 'a', config: app('a') }, { id: 'b', config: app('b') },
      ],
      edges: [
    ...
    expect(engine.getMetricsSnapshot().componentMetrics.lb.loadBalancerDistributionSkewPercent).toBeGreaterThan(0);

    const weightedOnly = new SysSimEngine({
      ...graph,
      nodes: graph.nodes.map((node) => node.id === 'lb'
        ? { ...node, config: loadBalancer({ stickySession: false, algorithm: 'weighted', targetWeights: { a: 3, b: 1 } }) }
        : node),
    }, traffic);
    const routes = Array.from({ length: 8 }, (_, index) => execute(weightedOnly, 'lb', index).path.at(-1)?.nodeId);
  ...
  it('reports unavailable-target failures separately', () => {
    const engine = new SysSimEngine({
      nodes: [{ id: 'lb', config: loadBalancer() }, { id: 'a', config: app('a', { health: 'down' }) }],
      edges: [{ id: 'a', source: 'lb', target: 'a', data: { protocol: 'HTTP', purpose: 'request' } }],
    }, traffic);
...
describe('API gateway tasks 100-104', () => {
  ...
  const graph = (gatewayConfig: ApiGatewayConfig, target = app('app')): SimGraph => ({
    nodes: [{ id: 'gateway', config: gatewayConfig }, { id: 'app', config: target }],
    edges: [{ id: 'route', source: 'gateway', target: 'app', data: { protocol: 'HTTP', purpose: 'request' } }],
  });

  it('enforces QPS and applies authentication-mode overhead', () => {
    const engine = new SysSimEngine(graph(gateway({ rateLimitQps: 1, authMode: 'OAuth2' })), traffic);
  ...
  it('enforces upstream timeout and reports timeout separately', () => {
    const engine = new SysSimEngine(graph(gateway({ timeoutMs: 5 }), app('app', { processingLatencyMs: 20 })), traffic);
  ...
  it('opens after three failures, fast-fails, then admits a half-open recovery probe', () => {
    ...
    expect(engine.getMetricsSnapshot().componentMetrics.gateway).toMatchObject({
      apiGatewayCircuitState: 'open', apiGatewayOpenCircuitRejections: 1,
    });
    target.health = 'healthy';
    engine.setGraph(circuitGraph);
    expect(execute(engine, 'gateway', 5, 10_020).status).toBe('success');
    expect(engine.getMetricsSnapshot().componentMetrics.gateway.apiGatewayCircuitState).toBe('closed');
  });
});
...
describe('CDN task 105', () => {
  ...
  const cdnGraph = (cdnConfig: CDNConfig): SimGraph => ({
    nodes: [
      { id: 'client-a', config: client('client-a') }, { id: 'client-b', config: client('client-b') },
      { id: 'cdn', config: cdnConfig }, { id: 'origin', config: app('origin') },
    ],
    edges: [
      { id: 'a-cdn', source: 'client-a', target: 'cdn', data: { protocol: 'HTTP', purpose: 'request' } },
      { id: 'b-cdn', source: 'client-b', target: 'cdn', data: { protocol: 'HTTP', purpose: 'request' } },
      { id: 'origin', source: 'cdn', target: 'origin', data: { protocol: 'HTTP', purpose: 'request' } },
    ],
  });
  ...
  it('uses hit target, TTL, edge locations, and optional origin shielding', () => {
    ...
    expect(execute(shielded, 'client-a', 1, 0, 'asset').path.some((hop) => hop.nodeId === 'origin')).toBe(true);
    expect(execute(shielded, 'client-b', 2, 0, 'asset').path.find((hop) => hop.nodeId === 'cdn')?.info).toContain('coalesced');

    const unshielded = new SysSimEngine(cdnGraph(cdn({ originShielding: false })), traffic);
    execute(unshielded, 'client-a', 1, 0, 'asset');
    expect(execute(unshielded, 'client-b', 2, 0, 'asset').path.some((hop) => hop.nodeId === 'origin')).toBe(true);
    unshielded.start();
    unshielded.step(20);
    expect(execute(unshielded, 'client-a', 3, 20, 'asset').path.find((hop) => hop.nodeId === 'cdn')?.status).toBe('hit');
    unshielded.step(1_100);
    expect(execute(unshielded, 'client-a', 4, 1_120, 'asset').path.find((hop) => hop.nodeId === 'cdn')?.status).toBe('miss');
  });
});
