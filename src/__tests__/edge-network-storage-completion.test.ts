...
const edge = (id: string, source: string, target: string, latencyMs = 4) => ({
  id,
  source,
  target,
  data: { protocol: 'HTTP' as const, purpose: 'request' as const, latencyMs },
});
...
describe('CDN tasks 106-108', () => {
  const graph = (locations: number): SimGraph => ({
    nodes: [
      { id: 'client', config: client('client') },
      {
        id: 'cdn',
        config: {
          ...(createDefaultConfig('cdn', 'cdn') as CDNConfig),
          edgeLocationsCount: locations,
          hitRatioPercent: 100,
          cacheTtlSec: 60,
        },
      },
  ...
  it('reports origin offload, fetch latency, and origin egress independently', () => {
    ...
    expect(
      engine.getMetricsSnapshot().componentMetrics.cdn.cdnOriginFetchLatencyMs,
    ).toBeGreaterThan(0);
  });
});

describe('DNS tasks 109-111', () => {
  const dns = (overrides: Partial<DNSConfig> = {}): DNSConfig => ({
    ...(createDefaultConfig('dns', 'dns') as DNSConfig),
    ttlSec: 1,
    lookupLatencyMs: 20,
    ...overrides,
  });
  ...
  it('caches resolutions by TTL and forwards application traffic to only the selected address', () => {
    ...
    expect(engine.getMetricsSnapshot().componentMetrics.dns).toMatchObject({
      dnsCacheHits: 1,
      dnsCacheMisses: 2,
    });
  });
  ...
  it('implements simple, weighted, geolocation, and latency-based policies deterministically', () => {
    ...
    const weighted = new SysSimEngine(
      graph(dns({ routingPolicy: 'weighted', targetWeights: { a: 3, b: 1 }, ttlSec: 1 })),
      traffic,
    );
    const weightedTargets = Array.from(
      { length: 8 },
      (_, index) =>
        execute(weighted, 'client', index, index * 1_001, `host-${index}`).path.at(-1)?.nodeId,
    );
    ...
    expect(execute(geo, 'client', 1, 0, 'one').path.at(-1)?.nodeId).toBe(
      execute(geo, 'client', 2, 2_000, 'two').path.at(-1)?.nodeId,
    );
  });
});

describe('WAF tasks 112-114', () => {
  const waf = (overrides: Partial<FirewallConfig> = {}): FirewallConfig => ({
    ...(createDefaultConfig('firewall', 'waf') as FirewallConfig),
  ...
  it('applies inspection plus documented rule cost and classifies malicious rejection', () => {
    const engine = new SysSimEngine(
      { nodes: [{ id: 'waf', config: waf({ blockRatePercent: 100 }) }], edges: [] },
      traffic,
    );
    ...
    expect(engine.getMetricsSnapshot().componentMetrics.waf).toMatchObject({
      wafBlockedRequests: 1,
      wafInfrastructureFailures: 0,
    });
  });

  it('reports infrastructure failure separately from WAF blocking', () => {
    const engine = new SysSimEngine(
      { nodes: [{ id: 'waf', config: waf({ health: 'down', blockRatePercent: 100 }) }], edges: [] },
      traffic,
    );
    expect(execute(engine, 'waf', 1).status).toBe('error');
    expect(engine.getMetricsSnapshot().componentMetrics.waf).toMatchObject({
      wafBlockedRequests: 0,
      wafInfrastructureFailures: 1,
    });
  });
});
...
describe('reverse proxy tasks 115-118', () => {
  const proxy = (overrides: Partial<ReverseProxyConfig> = {}): ReverseProxyConfig => ({
    ...(createDefaultConfig('reverse_proxy', 'proxy') as ReverseProxyConfig),
    maxConnections: 1,
    bufferSizeKb: 10,
    upstreamBandwidthMbps: 1,
    ...overrides,
  });
  ...
  it('enforces connections and models compression, bandwidth savings, buffering, and backpressure', () => {
    const engine = new SysSimEngine(
      {
        nodes: [
          { id: 'proxy', config: proxy() },
          { id: 'app', config: app('app', { processingLatencyMs: 100 }) },
        ],
...
describe('SQL tasks 119-120', () => {
  const sql = (): SqlDbConfig => ({
    ...(createDefaultConfig('sql_db', 'db') as SqlDbConfig),
    readReplicasCount: 2,
  });
  ...
  it('uses configured client workload ratio and routes reads to replicas while writes stay primary', () => {
    const readGraph: SimGraph = {
      nodes: [
        { id: 'client', config: client('client', { operationType: 'mixed', readPercentage: 100 }) },
        { id: 'db', config: sql() },
      ],
    ...
    reader.start();
    reader.step(1_000);
    expect(reader.getMetricsSnapshot().componentMetrics.db).toMatchObject({
      sqlReads: 2,
      sqlWrites: 0,
      sqlReplicaQueries: 2,
      sqlPrimaryQueries: 0,
    ...
    const writer = new SysSimEngine(
      {
        ...readGraph,
        nodes: [
          { id: 'client', config: client('client', { operationType: 'mixed', readPercentage: 0 }) },
          { id: 'db', config: sql() },
        ],
      },
      { ...traffic, baseQps: 1 },
    ...
    writer.start();
    writer.step(1_000);
    expect(writer.getMetricsSnapshot().componentMetrics.db).toMatchObject({
      sqlReads: 0,
      sqlWrites: 1,
      sqlReplicaQueries: 0,
      sqlPrimaryQueries: 1,
