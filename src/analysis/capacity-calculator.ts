...
const range = (expected: number, uncertainty: number, integer = false): EstimateRange => {
  ...
  return {
    low: transform(expected * (1 - uncertainty)),
    expected: transform(expected),
    high: transform(expected * (1 + uncertainty)),
  };
};
...
export function calculateCapacity(raw: CalculatorInputs): CalculatorOutputs {
  ...
  const readRequestPayloadKb = finite(
    raw.readRequestPayloadKb,
    DEFAULT_CAPACITY_ASSUMPTIONS.readRequestPayloadKb,
    0,
  );
  const readResponsePayloadKb = finite(raw.readResponsePayloadKb, writePayloadKb, 0);
  const writeResponsePayloadKb = finite(
    raw.writeResponsePayloadKb,
    DEFAULT_CAPACITY_ASSUMPTIONS.writeResponsePayloadKb,
    0,
  );
  const dbAverageServiceTimeMs = finite(
    raw.dbAverageServiceTimeMs,
    DEFAULT_CAPACITY_ASSUMPTIONS.dbAverageServiceTimeMs,
    0.1,
    60_000,
  );
  const dbTargetUtilizationPercent = finite(
    raw.dbTargetUtilizationPercent,
    DEFAULT_CAPACITY_ASSUMPTIONS.dbTargetUtilizationPercent,
    1,
    100,
  );
  const cacheWorkingSetDays = finite(
    raw.cacheWorkingSetDays,
    DEFAULT_CAPACITY_ASSUMPTIONS.cacheWorkingSetDays,
    0.01,
    retentionDays,
  );
  const cacheHotSetPercent = finite(
    raw.cacheHotSetPercent,
    DEFAULT_CAPACITY_ASSUMPTIONS.cacheHotSetPercent,
    0,
    100,
  );
  const cacheCompressionRatio = finite(
    raw.cacheCompressionRatio,
    DEFAULT_CAPACITY_ASSUMPTIONS.cacheCompressionRatio,
    0.01,
    1,
  );
  const targetUtilization =
    finite(
      raw.serverTargetUtilizationPercent,
      DEFAULT_CAPACITY_ASSUMPTIONS.serverTargetUtilizationPercent,
      1,
      100,
    ) / 100;
  const headroom =
    finite(raw.serverHeadroomPercent, DEFAULT_CAPACITY_ASSUMPTIONS.serverHeadroomPercent, 0, 500) /
    100;
  const failover =
    finite(
      raw.failoverCapacityPercent,
      DEFAULT_CAPACITY_ASSUMPTIONS.failoverCapacityPercent,
      0,
      500,
    ) / 100;
  const indexing =
    finite(
      raw.indexingOverheadPercent,
      DEFAULT_CAPACITY_ASSUMPTIONS.indexingOverheadPercent,
      0,
      500,
    ) / 100;
  const metadata =
    finite(
      raw.metadataOverheadPercent,
      DEFAULT_CAPACITY_ASSUMPTIONS.metadataOverheadPercent,
      0,
      500,
    ) / 100;
  const compression = finite(
    raw.storageCompressionRatio,
    DEFAULT_CAPACITY_ASSUMPTIONS.storageCompressionRatio,
    0.01,
    1,
  );
  const growth =
    finite(raw.annualGrowthPercent, DEFAULT_CAPACITY_ASSUMPTIONS.annualGrowthPercent, 0, 1_000) /
    100;
  ...
  const overheadAdjustedTb =
    rawRetentionTb * (1 + indexing + metadata) * compression * (1 + growth);
  const totalReplicatedStorageTb = overheadAdjustedTb * replicationFactor;
  const inboundBandwidthMbps =
    ((writeQps * writePayloadKb + readQps * readRequestPayloadKb) * 8) / 1_000;
  const outboundBandwidthMbps =
    ((readQps * readResponsePayloadKb + writeQps * writeResponsePayloadKb) * 8) / 1_000;
  ...
  const recommendedCacheMemoryGb =
    ((dailyNewDataGb * cacheWorkingSetDays * cacheHotSetPercent) / 100) * cacheCompressionRatio;
  const estimatedDbConnections =
    qps === 0
      ? 0
      : Math.ceil((qps * (dbAverageServiceTimeMs / 1_000)) / (dbTargetUtilizationPercent / 100));

  const outputs: CalculatorOutputs = {
    readQps: round(readQps, 2),
    writeQps: round(writeQps, 2),
    dailyNewDataGb: round(dailyNewDataGb),
    totalStorageNeededTb: round(overheadAdjustedTb, 2),
    totalReplicatedStorageTb: round(totalReplicatedStorageTb, 2),
    inboundBandwidthMbps: round(inboundBandwidthMbps),
    outboundBandwidthMbps: round(outboundBandwidthMbps),
    estimatedServersNeeded,
    recommendedCacheMemoryGb: round(recommendedCacheMemoryGb),
...
export function buildCapacityAssumptionsJson(
