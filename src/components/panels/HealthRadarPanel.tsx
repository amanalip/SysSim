...
const icons: Record<HealthPillarScore['name'], React.ReactNode> = {
  Availability: <ShieldCheck size={16} />,
  Scalability: <Zap size={16} />,
  'Modeled Latency': <Activity size={16} />,
  'Cost Efficiency': <DollarSign size={16} />,
  Resilience: <RefreshCw size={16} />,
};
  ...
  return (
    <div className={styles.container}>
      <ModelNotice
        ...
        assumptionLabel="documented health-score formulas"
        assumptionSection="health-scoring-formulas"
      />
      <div className={styles.header}>
        <div className={styles.titleGroup}>
          <span className={styles.title}>5-Pillar Heuristic Architecture Health Radar</span>
          <span className={styles.subtitle}>
            Evidence-aware design prompts; not an architecture certification
          </span>
        </div>
        <div className={styles.overallBadge}>
          <span className={styles.scoreNumber}>{overallAverage ?? 'N/A'}</span>
          <span className={styles.scoreLabel}>
            {overallAverage === null ? 'Awaiting evidence' : '/ 100 scored pillars'}
          </span>
        </div>
      </div>
          ...
          return (
            <div key={pillar.name} className={styles.pillarCard}>
              <div className={styles.cardHeader}>
                <div className={styles.pillarIcon} style={{ color: pillarColor }}>
                  {icons[pillar.name]}
                </div>
              ...
              <div className={styles.evidenceRow}>
                <span>{pillar.evidenceKind}</span>
                <span>Confidence: {pillar.confidence}</span>
                {pillar.sampleSize > 0 ? <span>n={pillar.sampleSize.toLocaleString()}</span> : null}
              </div>
              <div
                className={styles.progressTrack}
                aria-label={`${pillar.name} score ${pillar.score ?? 'not available'}`}
              >
                <div
                  className={styles.progressBar}
                  style={{ width: `${pillar.score ?? 0}%`, backgroundColor: pillarColor }}
                />
              </div>
