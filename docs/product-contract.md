...
| Compute, load balancing, gateways, and CDN | Global QPS is apportioned by client weights. Compute nodes model capacity, queueing, failure, and serverless lifecycle semantics. Load balancers implement five deterministic algorithms, health-check/recovery timing, stickiness, editable weights, connection lifetimes, and distribution/failure telemetry. API gateways enforce token-bucket QPS, authentication overhead, upstream timeout, and a documented circuit breaker with distinct counters. CDNs apply hit target, TTL, edge-location partitioning, and optional origin shielding. | Constants and formulas are explicit teaching assumptions rather than infrastructure benchmarks. Authentication does not contact a real identity provider, the circuit breaker is intentionally compact, and CDN geography is a deterministic edge partition rather than a world network model. | Use [Compute and Networking Component Semantics](component-semantics.md). Tasks 76–105 are complete. |
