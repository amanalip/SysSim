...
| Compute, edge networking, storage, and policy services | Global QPS is apportioned by client weights. Compute and edge nodes apply documented capacity, lifecycle, routing, caching, admission, compression, and buffering semantics. Storage components apply component-specific operation, latency, capacity, partition, quorum, payload, traversal, retention, and cold-tier assumptions. Rate limiting, authentication, and encryption apply explicit algorithm, cache, payload, and decision-latency assumptions with distinct telemetry. | Constants and formulas are explicit teaching assumptions rather than infrastructure or cryptographic benchmarks. Replicas, shards, partitions, and failover roles are virtual capacity inside one node. Token TTL and key rotation remain explicitly diagram-only. | Use [Component Semantics](component-semantics.md). Tasks 76–150 are complete. |
| Replicas and failover | A configured replica count is virtual capacity/redundancy inside one node and does not create independently routable instances. Load-balancer targets use interval checks and delayed recovery. A degraded SQL node can perform one simplified standby promotion when automatic failover is enabled; writes remain bound to the resulting single primary. | Orchestrator elections, split brain, replica loss, repair, and independently routable database members are not modeled. | Draw separate nodes when independent routing matters and treat internal failover telemetry as illustrative. |
