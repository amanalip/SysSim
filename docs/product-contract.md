...
| Compute | Global QPS is apportioned by client weights. App servers enforce per-replica connections and throughput with queueing, CPU load, and defined degraded behavior. Workers enforce replica/concurrency/rate/retry limits. Serverless functions model warm pools, idle-time cold probability, concurrency queues, timeouts, and memory-scaled execution. | Constants and formulas are teaching assumptions rather than infrastructure benchmarks; serverless throttling and downstream-failure attribution remain separate follow-up work. | Use [Compute Component Semantics](component-semantics.md). Tasks 76–90 are complete. |
