...
| Messaging | Message queues, task queues, pub/sub, and event buses create distinct logical delivery copies. Producer acknowledgement is separated from time-based consumer drain; worker capacity, populated partitions, consumer groups, subscriber fanout, delivery guarantees, retry backoff, DLQ, and ordering are modeled. | Membership/rebalancing, offset commits, retention expiry, broker persistence, transactional exactly-once processing, and outcome-specific telemetry are not complete. Exactly-once is simulated delivery-ID deduplication. | Use [Messaging Semantics](messaging-semantics.md). Tasks 61–70 are complete; follow-up messaging work remains in tasks 71–75. |
