...
| Compute and load balancing | Global QPS is apportioned by client weights. App servers enforce per-replica connections and throughput with queueing, CPU load, and degraded behavior. Workers enforce replica/concurrency/rate/retry limits. Serverless functions model warm pools, cold probability, throttling, timeouts, memory scaling, and failure attribution. Load balancers implement five deterministic algorithms over eligible targets and track routed connection lifetimes. | Constants and formulas are teaching assumptions rather than infrastructure benchmarks. Health-check delay, sticky sessions, editable weights, and unavailable-target telemetry remain follow-up work. | Use [Compute Component Semantics](component-semantics.md). Tasks 76–95 are complete. |
