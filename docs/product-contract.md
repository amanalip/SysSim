...
| Client and application server | Global QPS is apportioned by client weights; protocol, payload, operation type, and key distribution shape requests. App-server replicas add fixed-service slots and queueing without lowering intrinsic latency. | Protocol overhead constants and single-slot replicas are teaching assumptions. Connection limits, richer concurrency, CPU utilization, and degraded states remain. | Use [Client and Application Server Semantics](component-semantics.md). Tasks 76–80 are complete. |
