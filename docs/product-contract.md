...
| Cache behavior | Stable request keys, uniform/Zipfian/custom distributions, cache-aside termination/fallback/population, TTL, capacity, eviction, and cache-type scope are modeled. | Cacheability percentage is not a guaranteed realized hit ratio; outages, stampedes, coalescing, and metric reconciliation are not complete. | Use [Cache Semantics](cache-semantics.md). Remaining cache work is tracked in tasks 51–60. |
